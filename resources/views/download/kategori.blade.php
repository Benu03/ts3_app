                  ...
                  <tbody>
                     <?php $i=1; foreach($download as $download) { ?>
                      <tr style="border-bottom: solid thin #eee;">
                          <td width="80%">
                              <h5><a href="{{ secure_asset('dokumen/detail/'.$download->id_download.'/'.Str::slug($download->judul_download, '-')) }}" target="_blank"><?php echo $download->judul_download ?></a></h5>
                              <?php echo $download->isi ?>
                          </td>
                          <td class="td-top">
                              <a href="{{ secure_asset('dokumen/unduh/'.$download->id_download) }}" class="btn btn-info"><i class="fa fa-download"></i> Download</a>
                          </td>
                      </tr>
                      <?php $i++; } ?>
                   </tbody>
